...
void encrypt(){
	...
	char filler='X';
	bool filled=false;
	for(int i=0;i<in;i+=2){//filling x when two same alphabets are same
		reqText[k]=a[i];
		k++;
		if(i+1!=in){
	...
	int P,Q,R,S,f1,f2;
	char x,y;
	for(int i=0;i<in;i+=2){                         //separating 2 value and encrypting
		x=reqText[i];
		y=reqText[i+1];
		f1=f2=0;
		for(int j=0;j<5;j++){
